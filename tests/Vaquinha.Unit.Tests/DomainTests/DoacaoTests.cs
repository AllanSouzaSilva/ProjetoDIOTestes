...
{
    [Collection(nameof(DoacaoFixtureCollection))]  //Agrupar as coleções de testes  
                              ...
                              IClassFixture<CartaoCreditoFixture>
    {
        ...
        //Aceita pagar com taxa

         [Fact]
        [Trait("Doacao", "Doacao_UsuarioAceitapagarComTaxa_DoacaoValida")] //Organizar os testes
        ...
        {           
            ...
            // Assert
            doacao.Valor.Should().Be(6,because: "valor com taxade 20%");
        }
        ...
        public void Doacao_DadosPessoaisInvalidos_DoacaoInvalida()
        {
            ...
            // Act
            var valido = doacao.Valido(); //Vai executar a doação e ver se esta tudo valido

            // Assert
            //Assert.True(valido);
            valido.Should().BeFalse(because: "o campo email está inválido"); //Essa linhas quer dizer que os campos prescisar ser e inserido corretamentes, valido prescisa ser true
            doacao.ErrorMessages.Should().Contain("O campo Email é inválido.");
            doacao.ErrorMessages.Should().HaveCount(1, because: "somente o campo email está inválido.");//Ele joga todos os erros nessa variavel
        }

        [Theory] //Coloca essa notation e passar uma sequencia de teste pra ele, nesse caso estamos passando números proximos aos do limite que é < 0
        [InlineData(0)]
        [InlineData(-10)]
        [InlineData(-5)]
        [InlineData(-10.20)]
        [InlineData(-55.4)]
        [InlineData(-0.1)]
        [Trait("Doacao", "Doacao_ValoresDoacaoMenorIgualZero_DoacaoInvalida")] // Nesse cenário vai falhar pois os metodos  são invalidos
        public void Doacao_ValoresDoacaoMenorIgualZero_DoacaoInvalida(double valorDoacao)
        {
            ...
            doacao.ErrorMessages.Should().Contain("Valor mínimo de doação é de R$ 5,00"); //Messagem de erro
